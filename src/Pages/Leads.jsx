...
const Leads = () => {
  ...
  const leads = useSelector((state) => state.leads || []);
  const user = useSelector((state) => state.auth?.user || {});
  const leadStatuses = useSelector(
    (state) => state.settings?.leadStatuses || []
  );
  ...
  return (
    <div className="p-8 bg-[#f8fafc] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {user?.role === "admin" ? (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-100 text-blue-700 text-[10px] font-black uppercase">
                <ShieldCheck className="w-3 h-3" /> Admin View
              </span>
            ) : (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[10px] font-black uppercase">
                <UserIcon className="w-3 h-3" /> My Leads
              </span>
            )}
          </div>
          ...
          <p className="text-gray-500 text-sm font-medium">
            Manage, track, and update your sales pipeline.
          </p>
        </div>

        <button
          onClick={() => navigate("/add-lead")}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition-all active:scale-95"
        >
          <Plus className="w-5 h-5" />
          Create New Lead
        </button>
      ...
      {/* SEARCH + FILTER */}
      <div className="bg-white p-4 rounded-2xl border shadow-sm mb-6 flex flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-gray-400" />
          <select
            className="bg-gray-50 border rounded-xl px-4 py-2 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500"
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
          >
            <option value="All">All Statuses</option>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b">
              <th className="px-6 py-4 text-xs text-gray-400 uppercase">Prospect</th>
              <th className="px-6 py-4 text-xs text-gray-400 uppercase text-center">Type</th>
              <th className="px-6 py-4 text-xs text-gray-400 uppercase text-center">Status</th>
              <th className="px-6 py-4 text-xs text-gray-400 uppercase text-center">Follow Up</th>
              <th className="px-6 py-4 text-xs text-gray-400 uppercase text-right">Actions</th>
            </tr>
          </thead>
            ...
            {visibleLeads.length ? (
              visibleLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-blue-50/30">
                  ...
                  <td className="px-6 py-5 text-center">
                    <span className="px-3 py-1 bg-gray-100 rounded-lg text-xs font-black uppercase">
                      {lead.type || "N/A"}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-5 text-center">
                    <select
                      ...
                      className={`px-3 py-1.5 text-xs font-bold rounded-full border ${getStatusStyle(lead.status)}`}
                    >
                      {leadStatuses.map((status) => (
                  ...
                  <td className="px-6 py-5 text-center text-xs text-gray-500 font-mono">
                    <Calendar className="inline w-4 h-4 mr-1 text-blue-400" />
                    {lead.followUp || "Not set"}
                  </td>
                  ...
                  <td className="px-6 py-5 text-right">
                    <button
                      onClick={() => navigate(`/leads/${lead.id}`)}
                      className="p-2 rounded-lg bg-gray-50 hover:bg-blue-600 hover:text-white transition"
                    >
                      <Eye className="w-4 h-4" />
                    </button>
                  </td>
